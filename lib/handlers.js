...
handlers._checks.post =  function(data,callback){
    ...
    let timeoutSecond = typeof(data.payload.timeoutSecond) == 'number' && data.payload.timeoutSecond%1 == 0 && data.payload.timeoutSecond >= 1 && data.payload.timeoutSecond <= 5 ? data.payload.timeoutSecond : false
...
handlers._checks.get = function(data, callback){
    let phone = typeof(data.queryStringobject.id) == 'string' ? data.queryStringobject.phone : false
    let token = typeof(data.headers.authorization) == 'string' && data.headers.authorization.trim().length == 30 ? data.headers.authorization : false
    if(token){
        handlers._tokens.verifyToken(token, function(err){
            if(!err){
